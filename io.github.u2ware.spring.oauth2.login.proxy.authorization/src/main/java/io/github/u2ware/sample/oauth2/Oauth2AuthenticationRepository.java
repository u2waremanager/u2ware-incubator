...
@Component
public class Oauth2AuthenticationRepository implements AuthorizationRequestRepository<OAuth2AuthorizationRequest>{
    ...
    public void saveAuthorizationRequest(OAuth2AuthorizationRequest authorizationRequest, HttpServletRequest request, HttpServletResponse response) {
        ...
        String callback = request.getParameter(LOGIN_CALLBACK_PARAM);
        if(StringUtils.hasText(callback)){
            callbackRequests.put(key, request.getParameter(LOGIN_CALLBACK_PARAM));        
        }
        logger.info("\tsave request: "+key);
    }
    ...
    public OAuth2AuthorizationRequest removeAuthorizationRequest(HttpServletRequest request) {
        ...
        if(StringUtils.hasText(callback)){
            WebUtils.setSessionAttribute(request, LOGIN_CALLBACK_PARAM, callback);
            logger.info("\tsave callback: "+callback);
        }
        logger.info("\tremove request: "+key);
    ...
    public OAuth2AuthorizationRequest loadAuthorizationRequest(HttpServletRequest request) {
        ...
        String state = request.getParameter(OAuth2ParameterNames.STATE);
        if (state == null) {
            String callback = (String)WebUtils.getSessionAttribute(request, LOGIN_CALLBACK_PARAM);
            if(StringUtils.hasText(callback)){
                logger.info("\tload callback: "+callback);
