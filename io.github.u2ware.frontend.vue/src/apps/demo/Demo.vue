<template>
<div>
    <h1>Basic</h1>
    <h2><router-link to="/demo/basic">basic</router-link></h2>
    <h2><router-link to="/demo/basic-router">basic-router</router-link></h2>
    <h2><router-link to="/demo/basic-vuetify">basic-vuetify</router-link></h2>
    <h2><router-link to="/demo/basic-vuetify-layouts">basic-vuetify-layouts</router-link></h2>
    <h1>Test</h1>
    <h2><router-link to="/demo/test-components">test-components</router-link></h2>
    <h2><router-link to="/demo/test-components-events">test-components-events</router-link></h2>
    <h2><router-link to="/demo/test-i18n">test-i18n</router-link></h2>
    <h2><router-link to="/demo/test-axios">test-axios</router-link></h2>
    <h2><router-link to="/demo/test-moment">test-moment</router-link></h2>
    <h2><router-link to="/demo/test-froala">test-froala</router-link></h2>
    <h2><router-link to="/demo/test-tiptap">test-tiptap</router-link></h2>
    <h2><router-link to="/demo/test-ckeditor">test-ckeditor</router-link></h2>
    <h2><router-link to="/demo/test-jquery">test-jquery</router-link></h2>
    <h2><router-link to="/demo/test-summernote">test-summernote</router-link></h2>
</div>
</template>

<script>

export default {

}
</script>